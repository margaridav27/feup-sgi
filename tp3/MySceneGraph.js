...
import { MyPickableAnimation } from "./animations/MyPickableAnimation.js";
import { Game } from "./game/Game.js";
import { Menu } from "./game/menu/Menu.js";
import { Button } from "./game/menu/Button.js";
 ...
 */
export class State {
  static MENU = new State("menu");
  static GAME = new State("pick");
...
export class MySceneGraph {
  ...
  constructor(filename, scene) {
    ...
    this.game = undefined;
    this.menu = new Menu(this.scene, (options) => {
      this.game = new Game(this.scene, false, options, () => {
        this.state = State.MENU;
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      for (let j = 0; j < grandChildren.length; j++) {
        ...
        switch (grandChildren[j].nodeName) {
          ...
          default:
            this.onXMLMinorError(
              `Invalid material child tag (${grandChildren[j].nodeName}) for material = ${materialID}`
            );
        }
      }
      if (invalidGrandChild) continue;
   ...
   * @param {keyframes block element} animationComponent
  ...
  parseAnimations(animationsNode) {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      if (animationID === null || animationID === "") {
        this.onXMLMinorError("invalid ID defined for keyframeanim");
        continue;
      }

      let hasPickable = children[i].attributes["pickable"] !== undefined;
      const hasLoopAttr = children[i].attributes["loop"] !== undefined;
      const hasLoopTimeAttr = children[i].attributes["loop_time"] !== undefined;

      let hasLoop = hasLoopAttr && hasLoopTimeAttr;

      if (hasPickable && hasLoop) {
        this.onXMLMinorError(
          `an interface controllable animation cannot have a loop (conflict: ID = ${animationID})`
        );
      }

      // Get the loop and loop_time variables of the current animation if animation has loop.
      let animationLoop = false;
      let animationTime = null;
      // Testar meter outras cenas no loop
      if (hasLoop) {
        animationLoop = this.reader.getBoolean(children[i], "loop");
        if (animationLoop === null) {
          this.onXMLMinorError(
            `invalid value for loop variable of keyframeanim ${animationID} (must be either true or false)`
          );
          animationLoop = false;
        } else {
          ...
          if (isNaN(animationTime)) {
            this.onXMLMinorError(
              `invalid value for loop_time variable of keyframeanim ${animationID} (must be a numerical value)`
            );
            animationTime = null;
          }
        }
      ...
      let pickable = null;
      // Get the interface variable of the current animation.
      if (hasPickable) {
        pickable = this.reader.getBoolean(children[i], "pickable");
        if (pickable === null) {
          this.onXMLMinorError(
            `a pickable animation cannot have a loop (conflict: ID = ${animationID})`
          );
          hasPickable = false;
        }
      }
      ...
      this.animations[animationID] = hasPickable
        ? new MyPickableAnimation(keyframes)
        : new MyKeyframeAnimation(keyframes, animationTime);
    }
  ...
  parseComponents(componentsNode) {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      let newNode = new MyNode(i, componentID, mat4.create());
      ...
      const controlledAnimIndex = nodeNames.indexOf("controlledanim");
      ...
      // Check if component controls any animation - if so, make it pickable
      if (controlledAnimIndex !== -1) {
        const controlledAnimComponent = grandChildren[controlledAnimIndex];
        const controlledAnim = this.reader.getString(
          controlledAnimComponent,
          "id"
        );
        if (controlledAnim === null || controlledAnim === "") {
          this.onXMLMinorError(`\
            invalid id for tag <controlledanim> in component ${componentID} - \
            to make a component control an animation, its correspondent id must be passed to the tag`);
        }
        newNode.setControlledAnim(controlledAnim);
      }
  ...
  checkNodeTree(node, currentTree) {
    ...
    // TODO: check controlled animation
    const controlledAnim = node.getControlledAnim();
    if (
      controlledAnim !== null &&
      !this.usedAnimations.includes(controlledAnim)
    ) {
      console.log(controlledAnim, this.animations[controlledAnim]);
      this.usedAnimations = [...this.usedAnimations, controlledAnim];
    }
  ...
  processNode(
    ...
    const controlledAnim = node.getControlledAnim();
    if (controlledAnim !== null) this.scene.registerForPick(node.getID(), node);
    ...
    if (controlledAnim !== null) this.scene.clearPickRegistration();
        ...
        for (let i = 0; i < this.scene.pickResults.length; i++) {
          ...
          if (obj) {
            if (obj instanceof MyNode)
              this.animations[obj.getControlledAnim()].handlePick();
            else if (obj instanceof Button)
              this.menu.pickHandler(this.scene.pickResults[i]);
            else this.game.pickHandler(this.scene.pickResults[i]);
          }
        }
  ...
  displayScene() {
    ...
    if (this.state === State.MENU) this.menu.display();
    else if (this.state === State.GAME) this.game.display();
  }
   ...
   */
  update(t) {
    this.shader.setUniformsValues({ timeFactor: ((t / 1000) % 10) * 10 });
    
    if (this.usedAnimations !== undefined) {
      Object.values(this.usedAnimations).forEach((anim) =>
        this.animations[anim].update(t / 1000)
      );
    }
    ...
    if (this.state === State.GAME) {
      ...
      if (this.scene.gui.isKeyPressed("KeyZ")) 
        this.game.undo();
